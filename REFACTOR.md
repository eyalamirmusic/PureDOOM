...
- **Step 9 (modern C++ / RAII) opened**, after the goal was reframed (see the intro + the Step 9
  ...
  **persistent cluster owners** via the **unified owner/view recipe** — `GraphicsData`'s
  ...
  The **deep tail** left is the *nested* owners (`textures`/`sprites`, needing the vanilla struct
  types changed), `CompositeCache`'s tutti-frutti tables, the `Sim/Tick` level pool, and the
