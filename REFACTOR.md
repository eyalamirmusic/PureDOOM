...
state stays in its shim deliberately, the GPU automap in the app reading it. After these, the
remaining Step-5 work is the config rework (which unblocks the config-backed set), the `Thinker`
rewrite (which brings `thinkercap`), and the long file-scope-statics phase.
