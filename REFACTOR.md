...
the `Engine` (46 members in all), so `doomstat.h`/`r_state.h`/`p_local.h` are nearly empty of
...
(`StatusBarFace`/`StatusBarWidgets`/`StatusBarGraphics`/`StatusBarState`), with a new
`StateClusterTests` net pinning the golden-neutral par-time and movement-speed tables. What is still
left is config-backed-and-blocked,
...
`StatusBarFace`'s −1 sentinel, `StatusBarGraphics`' 42 faces, `StatusBarState`'s one-time gate), and
that the free accessors view the one `Engine`'s members (the guarantee `EngineTests` makes for
random/wad/level). Shown to bite: corrupting E1M1's par (30 → 31) fails `parTimesMatchVanilla`.
...
- **The globals-into-`Engine` work** (Step 5) — **the loose globals are nearly all in, and
  ...
  empty: ~25 cohesive clusters have moved into `Engine` (46 members in all — see "the game state
  ...
  - **The rest of the file-scope statics** (~1,534 in all) and the remaining Game-local globals,
    ...
    statics, with a `StateClusterTests` net for the golden-neutral tables. This is what finally lets
    the engine be *constructed* rather than booted, once nothing reaches state by free function.
