...
the `Engine` (47 members in all), so `doomstat.h`/`r_state.h`/`p_local.h` are nearly empty of
...
view state (`AutomapView`), with a new `StateClusterTests` net pinning the golden-neutral par-time
...
- **The globals-into-`Engine` work** (Step 5) — **the loose globals are nearly all in, and
  ...
  empty: ~25 cohesive clusters have moved into `Engine` (47 members in all — see "the game state
  ...
  - **The rest of the file-scope statics** (~1,534 in all) and the remaining Game-local globals,
    ...
    statics, then the automap's own view state (`AutomapView`), with a `StateClusterTests` net for
    the golden-neutral tables. This is what finally lets the engine be *constructed* rather than
    booted, once nothing reaches state by free function.
