...
| 9 | Modern C++ / RAII across the board — the second half of the goal | **all three strands done; one audio-blocked item** — Steps 4–5 made the state *owned*; this makes it *read like C++ someone wrote*. Three strands: (a) retire the reference-alias layer so every reader reaches state through an owner, which unpins the `Engine`'s address; (b) the RAII sweep over the manual `doom_malloc`/`doom_free` owners; (c) the idiom cleanup over code already in `namespace Doom`. **(c) is done** — `#pragma once`, `typedef struct {…} T;` → `struct T {…};` (106 sites), C arrays → `EA::Array` (**now genuinely all of them** — this row twice claimed it finished while it had not been counted; the category was enumerated at 130 members across 43 headers, 111 converted in three batches and 19 correctly left raw, see item 8), `(void)` parameter lists (326), pointer→reference where provably safe (**109 functions, now swept in five batches** — the specials family, the playsim core, the weapon and enemy actions, the seven `Event*` responders and the automap's line helpers; the nullability established per call site by three surveys rather than assumed, and the ~11 genuinely-nullable parameters left as pointers and tabulated in `CLAUDE.md`. The sweep's one real finding was a **by-value lambda capture of a now-reference parameter** in `startButton`, which copied a whole `Line` and silently broke a de-duplication test that all 98 tests passed straight over) — as is the whole vanilla-name retirement: no prefixed function name and no `_t` type survives, the flat layer is gone, and `fixed_t`/`angle_t` **are** `Doom::Fixed`/`Doom::Angle`. **(a) is done**, after being declared done twice while a whole syntactic tier stood: 247 file-local aliases fell in seven batches, and on that the **`Engine` is constructible** with a test that proves it. `MovementSpeeds`/`VideoState::dirtybox` are `int`; the hitscan traversers return `AimResult`; `UI/Automap` and `UI/Finale` gained the frame goldens they never had. **(b) is now done bar one audio-blocked item**: the eight remaining manual `doom_malloc`/`doom_free` owners are RAII-owned (`UI/Wipe`'s melt offsets, `UI/Intermission`'s `lnames`, `Game/Game`'s save and demo buffers, `Game/Config`'s string defaults, `DoomMain`'s seven WAD paths + `addWadFile`), `readFile`'s `byte**` out-parameter is retired, and the dead `Host/System` zone vestiges are deleted — and the level pool has a destructor now (it was leaking every mobj on `resetEngine()`), and the last two — `Host/Sound`'s `paddedsfx` (which was not merely audio-blocked but leaking ~107 buffers per boot, `initSoundHost` running at boot) and `findResponseFile`/`myargv` (now a `std::vector<std::string>`, which took two leaks, a one-byte overflow and two silent ceilings with it) — have since landed too. `AutomapView::min_w`/`min_h` were investigated against the 1993 source and deleted as an id leftover, not a lost read. **`doom_boolean` is now done too** — all ~288 uses are a real `bool` and the typedef is deleted, in four verified batches (Render 19, Host 4, UI 104, Sim+Game 161), leaving four deliberate `int`s that only look like flags. **The function-like macro layer is now retired too** — `Math/Swap.h`'s `SHORT`/`LONG` across 154 sites became one deduced-width `Doom::littleEndian`, and `UI/Automap`'s seven, `Sim/SaveGame`'s `PADSAVEP` and `UI/CheatTypes`' `SCRAMBLE` became functions; `Host/Net`'s `ntohl`/`ntohs` stay, deliberately, being under an `#if` for a platform no gate here compiles. That sweep found the refactor's **one real behaviour bug** (`thintriangle_guy`, see the traps section) and two more little-endian-invisible defects. **The constant macros followed and are now essentially closed — 629 → 309 across `src/DOOM`**, of which 199 of the remainder is `Game/StringsFrench.h`, which no build here compiles: `Sim/SimDefs.h`, `Sim/SpecialTypes.h`, `Render/Lighting.h`, `UI/Hud.h`, `UI/HudWidgetTypes.h`, `UI/StatusBarTypes.h`, 290 of `Game/StringsEnglish.h`'s 293, then `Game/GameDefs.h`, `UI/AutomapTypes.h`, `Wad/MapFormat.h`, `Game/NetTypes.h`, `UI/StatusWidgetTypes.h`, `Sim/WeaponTypes.h` and every `.cpp`-local pile. **That sweep's largest finding was not about macros**: eleven constants existed twice, once as the vanilla macro and once as a `constexpr` on the owning `Engine` cluster, and in five of them the overflow guard tested one while the array was sized by the other — see item 6. `Math/TrigTables.h` was the same thing at eleven-in-one-file scale and became a deletion rather than a conversion. **The warning count went 81 → 1 → 0**, the last survivor (the deliberate type-erased-action cast) having moved behind a scoped per-compiler suppression — and that zero is now measured on **three** configurations rather than one, GCC 16 included. GCC had never been built here: it failed outright (CMake's C++20 module scanning breaks Apple's SDK headers under GCC) and then reported 327 warnings, 314 of them hidden from Clang behind a pragma spelled in Clang's own dialect. Two real defects fell out of that, both preserved and documented. The runtime-accessor macros are converted too, the list of six having been wrong in both directions. **See "Where Step 9 actually stands" in the handoff for the authoritative state** — this row is a summary, and the handoff is where the detail and the traps live. |
| 10 | Strings: `std::string`/`std::string_view` everywhere, and the `extern "C"` API removed | **done, this session** — the whole C string layer is gone: `doom_strcpy`/`doom_concat`/`doom_itoa`/`doom_ctoa`/`doom_ptoa`/`doom_strlen`/`doom_strcmp`/`doom_strncmp`/`doom_strcasecmp`/`doom_strncasecmp`/`doom_strncpy`/`doom_fprint`/`doom_atoi`/`doom_atox`/`doom_toupper` are deleted, replaced by `Host/Text.h` (`concat`/`print`/`printTo`/`hexString`/`nameView`/`fillField`/`toUpper`/`equalsIgnoreCase`/`parseInt`/`parseHex` — the last two exact ports of the sign-less vanilla parsers, which the goldens pin) and a variadic `fatalError`. The global `error_buf[260]` and its ~250 strcpy/concat/abort chains across 18 files are single `fatalError(...)` calls; the ~180 `doom_print` piece-chains are single `print(...)` calls; every lump-name composition (`"STTNUM"+i`, `"WIMAP"+epsd`, `"ds"+sfx`, `E?M?`, `SAVEGAMENAME+slot`) is a `concat(...)` fed straight to the WAD lookups, which take `std::string_view` (`WadFile::find/number/addFile`, `cacheLumpName`, `textureNumForName`/`flatNumForName` — with `nameView(ptr, 8)` guarding the NOT-NUL-terminated 8-byte lump-name fields, the one new over-read hazard `string_view` introduces). State went owning: `ConfigPaths`/`SaveGameState::name`/`DemoState::demoname`/`NetState::exitmsg`/`MenuState` (savegamestrings + editor on `std::string`, `saveCharIndex` retired)/`HudTextLine::l` (with `len` retired)/`FinaleState` text views/`wadfiles` as `EA::Vector<std::string>`; the name tables (`sprnames`, `alphSwitchList`, `animdefs`, menu items/custom texts, `skullName` family) are `std::string_view`. Savegame on-disk format unchanged: fixed 24/16-byte fields written zero-padded via `fillField`, read back bounded. **And the `extern "C"` API is removed at the user's direction**: `DOOM.h` is a C++ header in `namespace Doom` (`initGame`/`updateGame`/`forceUpdateGame`/`framebuffer`/`soundBuffer`/`tickMidi`/`keyDown`/…/`setDefaultInt`), `doom_key_t`/`doom_button_t`/`doom_seek_t` are `Doom::Key`/`Doom::MouseButton`/`Doom::SeekOrigin` (enumerator spellings kept, now namespaced), and the 13 host callbacks are `std::function` members of `Doom::host()` with working defaults installed by `Host::Host()` (stdio/`gettimeofday`/`eacp::getEnv`; the `DOOM_IMPLEMENT_*` gates deleted) — embedders assign members directly, the `doom_set_*` setters are gone, and the string-shaped hooks take `std::string_view` (`getenv` returns `std::optional<std::string>`). Deliberately not converted: the cheat sequences (mutable `unsigned char` streams with in-band markers), `Wad/MapFormat.h` overlays, `Player`'s memcpy'd arrays, `doom_memset`/`doom_memcpy`, the phase-6 string-literal macro families. Two lineage bugs found and preserved, not fixed: quicksave's and quickload's third builder call is `strcpy` where `concat` was meant, so the prompt never shows the savegame name. All goldens byte-identical throughout; zero warnings held. **A second pass finished the job**: the first left ~363 raw `const char*`/`char const*` declarations standing, now **10, four of them comments**. Converted: the file-I/O and name-taking parameters (`readFile`/`writeFile`/`WritePCXfile`/`getsfx`/`addMessageToSText`, the probe's boot and lookup entry points), the static name and message tables (`mapnames`+3 siblings, `player_names`, `gammamsg`, `endmsg`, `chat_macros`, `SfxInfo::name`, `MusicInfo::name`, `Finale`'s 22 per-ending text pointers), the config table's text plumbing (`ConfigDefault::name`/`text_location`/`default_text_value`), `MenuState::messageString`, `AttractMode::pagename`, and all **287** of `StringsEnglish.h`'s constants as `constexpr std::string_view` (every one of the 500 literals diffed before/after and byte-identical). **`Player::message` went too** — the standing reason not to (`Player` is memcpy'd whole) only rules out an *owning* type; a view is trivially copyable, and `unArchivePlayers` clears it after the memcpy anyway. `StringsFrench.h` deliberately untouched (still `#define`s, compiled by no build). **A third pass has since taken the text-carrying `char*` count to zero**: `addPrefixToIText` (the dead-in-both-eras exemption, which did not survive scrutiny — that argument was borrowed from the reverted `[[maybe_unused]] constexpr`s and does not transfer to a parameter type), `extractFileBase` and `doomSimLumpName` (out-parameters, now returning `std::string`), `getParam`, `checkTable` and `DemoReplay`'s `subject`. `CheatSequence` became a `std::span<unsigned char>` + `int position` — the "it is written into" objection ruled out an owning immutable type, not a typed view — and got `Tests/Sim/CheatTests.cpp`, four cases over code that had **no coverage at all**, demonstrated sharp against two mutations. Every `char*` now left in the repository is a byte cursor, an OS entry-point signature or a cast a C API demands; `UI/Hud.cpp`'s `shiftxform` stays raw on purpose, because as a `std::span` its (vanilla, latent) out-of-range chat read would become an MSVC debug-STL assertion. **The one real hazard was a `std::string_view` built from a literal `0`**, which is UB and segfaults far from the edit — `S_music[0]`'s `{ 0 }` dummy name at static-init, and `doomSimBoot(0)` at 13 test call sites (16 tests down at once); the probe's parameter is defaulted now rather than passed `""`. Also fixed en route: three stale `const char*` forward declarations in `Render/Data.cpp` shadowing `Data.h`'s `string_view` ones, two `const_cast<char*>`s in `UI/Automap.cpp`, a stale `(char*)` cast in the probe, and four headers (`AttractMode.h`, `PlayerTypes.h`, `MenuState.h`, `FinaleState.h` — the last already broken before this change) that relied on a transitive `<string_view>` Apple Clang supplied and GCC did not. Verified on Apple Clang `Debug`+`Release`, GCC 16 `Release` and the eacp app: 94/94 then (98/98 after the third pass added `CheatTests`), zero warnings in `src/DOOM`, no golden re-recorded. |
...
1. **The RAII tail — now down to two blocked items.** ~~Six more~~ *(landed this
   ...
   **What is left: nothing.** Both items this entry called blocked have landed, and
   both were misdiagnosed in the same way — as *container-swap* problems, when each
   was a question about who owns the storage.

   - `Host/Sound`'s `paddedsfx` was filed as "audio-blocked and therefore
     golden-blind". Half true: no gate can hear it, but `initSoundHost` is called
     from `Host/System.cpp` at **boot**, not when audio starts, so it was leaking
     ~107 buffers on every single boot of every single test. Golden-blind is not
     the same as inert. It is a `std::vector<std::vector<unsigned char>>` beside the
     table now — not *in* `SfxInfo::data`, because a linked sound deliberately
     aliases its target's buffer and an owning member would have copied it.
   - `findResponseFile`/`myargv` was filed as needing "`myargv`'s own ownership
     rethought, not a container swap" — correct, and the rethink is one line:
     `myargv` is a `std::vector<std::string>`. The in-place tokenisation that made a
     scope-owned buffer dangle stops mattering once the arguments own their own
     characters. It also freed `Tests/SimProbe.cpp` of three `static char[]`s that
     existed solely to outlive the boot call.

     Doing it surfaced three defects the entry had not suspected, none reachable by
     any gate here (no test drives `@responsefile`): two unfreed `doom_malloc`s, a
     one-byte write past the contents buffer when the last token ran to EOF, and two
     silent ceilings (`MAXARGVS` = 100 tokens, 20 trailing arguments) that overran
     rather than reporting. `myargc` is `myargCount()` now, derived from the vector,
     so the count cannot drift from the container — item 6's rule about guards and
     bounds sharing a token, applied to a different pair.
...
6. **The macro layer.** The numbered list above was written when the open work was
   ...
   - ~~**The string tables**~~ — **done for the half this repository compiles.**
     ...
     *(Superseded by the row-10 second pass: `gammamsg` is
     `EA::Array<std::string_view, 5>`, its reader `Player::message` being a view now.)*
...
7. ~~**Zero the warning count, and treat it as a gate.**~~ **Done — the engine
   ...
   - ~~**59 × `-Wwritable-strings`**~~ — all in `UI/Hud.cpp`: `chat_macros[]`,
     ...
     *(Superseded by the row-10 second pass: all three, and `chat_macros`, are
     `EA::Array<std::string_view, N>`; `ConfigDefault::text_location` is
     `std::string_view*`. The reasoning below still explains **why** the pointers
     had to be the mutable half — that has not changed, only the element type.)*
   ...
   **GCC is now measured, and the prerequisite above was right to insist on it.**
   ...
   now *(and `std::string_view` after the row-10 second pass)*, and both pragma lines
   are deleted — the same fix this item already records
   ...
   **A third copy of that pragma, in `Game/Config.cpp`, was suppressing nothing at
   all.** `ConfigDefault::name` had become `const char*` at some earlier point
   *(`std::string_view` since the row-10 second pass)*, so the line named a warning
   the table could no longer raise. It is deleted. **A
...
- **The deep tail is now mostly cleared** (this session, each golden-verified): (1)
  **`CompositeCache`'s five per-texture composition tables** — `texturewidthmask`/
  ...
  behaviour-preserving by construction. **What was left of the deep tail:** the `Host/Sound` per-sfx
  padding cache (`paddedsfx`, a `+8`-offset per-sound cache, called audio-blocked and
  therefore golden-blind — **since done**, once it turned out `initSoundHost` runs at boot
  and it was leaking ~107 buffers on every test), and the level pool (`Sim/Tick`'s intrusive-list, variable-sized,
