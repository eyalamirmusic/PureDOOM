...
the `Engine` (39 members in all), so `doomstat.h`/`r_state.h`/`p_local.h` are nearly empty of
...
(`PendingCommands`) have moved so far. What is still left is config-backed-and-blocked,
...
- **The globals-into-`Engine` work** (Step 5) — **the loose globals are nearly all in, and
  ...
  empty: ~25 cohesive clusters have moved into `Engine` (39 members in all — see "the game state
  ...
  - **The rest of the file-scope statics** (~1,534 in all) and the remaining Game-local globals,
    ...
    into `NetState`, `ParTimes`, `MovementSpeeds`, `TimeDemo`, `PendingCommands`) and continues
    into the UI/renderer/specials files. This is what finally lets the engine be *constructed*
