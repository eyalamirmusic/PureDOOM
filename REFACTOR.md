...
- **Step 9 (modern C++ / RAII) opened**, after the goal was reframed (see the intro + the Step 9
  ...
  **persistent cluster owners** via the **unified owner/view recipe** — `GraphicsData`'s
  ...
  unchanged); the vanilla `spritedef_t`/`texture_t` structs now hold owned vectors. The **deep tail**
  left is `CompositeCache`'s tutti-frutti column/composite tables, the `Sim/Tick` level pool, and the
  remaining host buffers (`screens[]`).
