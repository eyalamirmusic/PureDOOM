...
the `Engine` (50 members in all), so `doomstat.h`/`r_state.h`/`p_local.h` are nearly empty of
...
runtime state (`FinaleState`) and the melt's scratch framebuffers (`WipeState`), with a new
...
- **The globals-into-`Engine` work** (Step 5) — **the loose globals are nearly all in, and
  ...
  empty: ~25 cohesive clusters have moved into `Engine` (50 members in all — see "the game state
  ...
  - **The rest of the file-scope statics** (~1,534 in all) and the remaining Game-local globals,
    ...
    runtime state (`FinaleState`, its immutable text/cast reference data left file-local) and the
    melt's scratch framebuffers (`WipeState`), with a `StateClusterTests` net for the golden-neutral
