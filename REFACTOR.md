...
byte-identical. What is left of the tail: `save_p` (p_saveg's serialization cursor, which moves
with the `Thinker`/p_saveg rewrite), and the boot-time file/path strings in `Game/DoomMain.cpp`
(`wadfiles[]`, `wadfile[]`, `mapdir[]`, `title[]` — read only within DoomMain, and
`basedefault[]` config-blocked). The `am_map` automap state stays in its shim deliberately, the
GPU automap in the app reading it. After these, the remaining Step-5 work is the config rework
(which unblocks the config-backed set), the `Thinker` rewrite (which brings `thinkercap`), and
the long file-scope-statics phase.
