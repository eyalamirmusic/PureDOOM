...
menu_t MainDef = {main_end, 0, MainMenu, M_DrawMainMenu, 97, 64, 0};
...
menu_t OptionsDef = {opt_end, &MainDef, OptionsMenuFull, M_DrawOptions, 60, 37, 0};
...
menu_t OptionsNoMouseDef = {
    opt_end_no_mouse, &MainDef, OptionsMenuNoMouse, M_DrawOptions, 60, 37, 0};
...
menu_t OptionsNoSoundDef = {
    opt_end_no_sound, &MainDef, OptionsMenuNoSound, M_DrawOptions, 60, 37, 0};
...
menu_t MouseOptionsDef = {
    mouse_opt_end, &OptionsDef, MouseOptionsMenu, M_DrawMouseOptions, 60, 70, 0};
...
menuitem_t SoundMenuFull[] = {{2, "M_SFXVOL", M_SfxVol, 's'},
                              {-1, "", 0},
                              {2, "M_MUSVOL", M_MusicVol, 'm'},
                              {-1, "", 0}};

menu_t SoundDef = {sound_end, &OptionsDef, SoundMenuFull, M_DrawSound, 80, 64, 0};
...
menuitem_t SoundMenuNoSFX[] = {{2, "M_MUSVOL", M_MusicVol, 'm'}, {-1, "", 0}};

menu_t SoundNoSFXDef = {
    sound_end_no_sfx, &OptionsDef, SoundMenuNoSFX, M_DrawSound, 80, 64, 0};
...
menuitem_t SoundMenuNoMusic[] = {{2, "M_SFXVOL", M_SfxVol, 's'}, {-1, "", 0}};

menu_t SoundNoMusicDef = {
    sound_end_no_music, &OptionsDef, SoundMenuNoMusic, M_DrawSound, 80, 64, 0};
...
menuitem_t DOOM_LoadMenu[] = {{1, "", M_LoadSelect, '1'},
                              {1, "", M_LoadSelect, '2'},
                              {1, "", M_LoadSelect, '3'},
                              {1, "", M_LoadSelect, '4'},
                              {1, "", M_LoadSelect, '5'},
...
menu_t LoadDef = {load_end, &MainDef, DOOM_LoadMenu, M_DrawLoad, 80, 54, 0};

//
// SAVE GAME MENU
//
menuitem_t SaveMenu[] = {{1, "", M_SaveSelect, '1'},
                         {1, "", M_SaveSelect, '2'},
                         {1, "", M_SaveSelect, '3'},
                         {1, "", M_SaveSelect, '4'},
                         {1, "", M_SaveSelect, '5'},
    ...
    for (int i = 0; i < custom_texts_count; ++i)
    {
        ...
        if (doom_strcmp(custom_text->name, name) == 0)
        {
            ...
            while (seg->lump)
            {
                ...
                V_DrawPatchRectDirect(
                    x + seg->offx, y, 0, (patch_t*) (lump), seg->x, seg->w);
                ++seg;
            }
            break;
        }
...
void M_DrawSaveLoadBorder(int x, int y)
{
    ...
    V_DrawPatchDirect(
        x - 8, y + 7, 0, (patch_t*) (W_CacheLumpName("M_LSLEFT", PU_CACHE)));

    for (i = 0; i < 24; i++)
    {
        V_DrawPatchDirect(
            x, y + 7, 0, (patch_t*) (W_CacheLumpName("M_LSCNTR", PU_CACHE)));
        x += 8;
    }

    V_DrawPatchDirect(
        x, y + 7, 0, (patch_t*) (W_CacheLumpName("M_LSRGHT", PU_CACHE)));
}
...
void M_DrawReadThis1(void)
{
    ...
    switch (gamemode)
    {
        case commercial:
            V_DrawPatchDirect(
                0, 0, 0, (patch_t*) (W_CacheLumpName("HELP", PU_CACHE)));
            break;
        case shareware:
        case registered:
        case retail:
            V_DrawPatchDirect(
                0, 0, 0, (patch_t*) (W_CacheLumpName("HELP1", PU_CACHE)));
            break;
        default:
            break;
    }
...
void M_DrawReadThis2(void)
{
    ...
    switch (gamemode)
    {
        ...
        case commercial:
            ...
            V_DrawPatchDirect(
                0, 0, 0, (patch_t*) (W_CacheLumpName("CREDIT", PU_CACHE)));
            break;
        case shareware:
        case registered:
            V_DrawPatchDirect(
                0, 0, 0, (patch_t*) (W_CacheLumpName("HELP2", PU_CACHE)));
            break;
        default:
            break;
    }
...
{
    V_DrawPatchDirect(
        108, 15, 0, (patch_t*) (W_CacheLumpName("M_OPTTTL", PU_CACHE)));
...
void M_DrawMouseOptions(void)
{
    ...
    V_DrawPatchDirect(MouseOptionsDef.x + 149,
...
void M_DrawThermo(int x, int y, int thermWidth, int thermDot)
{
    ...
    {
        V_DrawPatchDirect(
            xx, y, 0, (patch_t*) (W_CacheLumpName("M_THERMM", PU_CACHE)));
        xx += 8;
    }
...
void M_Drawer(void)
{
    ...
    for (i = 0; i < max; i++)
    {
        ...
        if (menuitem->name[0])
        {
            ...
            {
                V_DrawPatchDirect(
                    x, y, 0, (patch_t*) (W_CacheLumpName(menuitem->name, PU_CACHE)));
            }
        }
        y += LINEHEIGHT;
    }
