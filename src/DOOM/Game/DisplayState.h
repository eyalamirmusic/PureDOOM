...
namespace Doom
{
// D_Display's frame-diff state: the previous frame's overlay/view flags and the border-redraw
...
// These were dDisplay's own function-local statics, read by no other function; vanilla never resets
// them (they persist for the process), so each becomes a member with a matching default reached by a
// local reference in dDisplay - identical persistence in a single-Engine process. dDisplay runs
...
struct DisplayState
{
    ...
    gamestate_t oldgamestate = (gamestate_t) (-1); // last drawn gamestate, -1 forces a redraw
