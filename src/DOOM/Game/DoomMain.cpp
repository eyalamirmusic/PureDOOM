...
namespace Doom
{
// The boot-time WAD list, file-local: addWadFile appends to it and initWadFiles
// consumes it, and nothing outside this file reads it (its d_main.h extern is gone).
static Vector<std::string> wadfiles;

// The command-line launch flags are a LaunchOptions owned by the Engine now; these
// are references onto it (REFACTOR.md, Step 5).

// (drone, an always-unread vestige, was deleted here - the DoomCom.drone in d_net.h
// is an unrelated struct member.)

// singletics / debugfile are EngineParams owned by the Engine now; these are references onto
// it (REFACTOR.md, Step 5).

// The new-game defaults are a StartupDefaults owned by the Engine now; these are
// references onto it (REFACTOR.md, Step 5).

// advancedemo is a AttractMode member owned by the Engine now; this reference stays
...
//
// The input event ring buffer is a EventQueue owned by the Engine now; these vanilla
// names are references onto it (events[] as a reference-to-array).

// wipegamestate (with gamestate) is a GameFlow owned by the Engine now; this is a
// reference onto it. It can be set to -1 to force a wipe on the next draw.
void executeSetViewSize();

// Title/demo-loop state: a member of the AttractMode above, still read by g_game
// through its own extern declaration.

// Other subsystems' functions this file reaches. statcopy is defined in
// Game/Game.cpp; no header declares it, so this file forward-declares it.
void*& statcopy();
void checkNetGame();
