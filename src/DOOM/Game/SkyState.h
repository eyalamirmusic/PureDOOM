...
namespace Doom
{
// The lump number of the sky flat (F_SKY1). initFlats resolves it once at startup, and it
...
// vertical offset initSkyMap sets it to be drawn at). All three are "the sky" and move together.
struct SkyState
{
    int skyflatnum = 0; // lump number of the sky flat (F_SKY1)
    int skytexture = 0; // texture number of the current map's sky
    Fixed skytexturemid {}; // vertical offset the sky is drawn at
};
