...
namespace Doom
{
// The last thing that made noise, propagated to nearby monsters as P_NoiseAlert / P_RecursiveSound
...
// said "global because p_saveg archives it" - but p_saveg only ever touches sector_t::soundtarget
...
struct SoundTarget
{
    mobj_s* soundtarget = nullptr; // the noise source the alert is propagating from
};
