...
// both the head and tail of the thinker list; a reference onto ThinkerList's cap
...
// The item respawn queue lives in ItemRespawnQueue (an Engine member) now; these are
...
// P_MAPUTL
//
// The utilities themselves are * now - the pure geometry in
...
// traversal). Vanilla's divline_t went with them: it is DivLine, the same
...
// intercepts[] and intercept_p moved into Clip (Sim/Clip.h), reached through
// clipping(); they were p_maputl's own scratch, read by no other file.
...
using Traverser = FunctionRef<bool(Intercept*)>;
...
// These are references into Clip now (see Sim/Movement.h); p_enemy reads
